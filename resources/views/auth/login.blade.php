    ...
    <form method="POST" action="{{ route('login') }}" class="w-[400px] mx-auto p-6 my-16">
        ...
        <div class="flex justify-between items-center mb-5">
            ...
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-sm text-yellow-700 hover:text-yellow-600">
                    Forgot Password?
                </a>
            @endif
        </div>
        <button
            class="btn-primary bg-yellow-400 hover:bg-yellow-500 active:bg-yellow-600 w-full"
        >
            Login
        </button>
    </form>
